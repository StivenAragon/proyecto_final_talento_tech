...
export const seedUsuarios = async () => {
    ...
    const usuarios = [
        { nombre: "Super Administrador", email: "super_administrador@gmail.com", password: hashedPasswordSuperAdmin,  rol_id: 1, },
        { nombre: "Administrador", email: "administrador@gmail.com", password: hashedPasswordAdmin,  rol_id: 2, },
    ];
    ...
    for (const usuario of usuarios) {
        ...
        if (rows[0].count === 0) {
            const [roleRows] = await pool.query("SELECT id FROM roles WHERE id = ?", [usuario.rol_id]);
            if (roleRows.length === 0) {
                continue;
            }
            const roleId = roleRows[0].id;

            await pool.query( "INSERT INTO usuarios (nombre, email, password, rol_id) VALUES (?, ?, ?, ?)", [usuario.nombre, usuario.email, usuario.password, roleId] );
        }
    }
};
